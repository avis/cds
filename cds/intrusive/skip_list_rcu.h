...
#   endif // ifndef CDS_CXX11_LAMBDA_SUPPORT
    ...
    protected:
            ...
            for ( int nLevel = (int) c_nMaxHeight - 1; nLevel >= 0; --nLevel ) {
                ...
                while ( true ) {
                    ...
                    if ( pSucc.bits() ) {
                        ...
                        if ( !pPred->next( nLevel ).compare_exchange_strong( p, marked_node_ptr( pSucc.ptr() ), memory_model::memory_order_release, CDS_ATOMIC::memory_order_relaxed ))
                            goto retry ;
                        if ( nLevel == 0 ) {
                            // We cannot free the node at this moment
                            // since RCU is locked
                            // Link deleted nodes to chain to free later
        ...
        bool try_remove_at( node_type * pDel, Q const& val, position& pos, Func f )
        {
            ...
            for ( unsigned int nLevel = pDel->height() - 1; nLevel > 0; --nLevel ) {
                while ( true ) {
                    ...
                    if ( pDel->next(nLevel).compare_exchange_weak( pSucc, pSucc | 1, memory_model::memory_order_acquire, CDS_ATOMIC::memory_order_relaxed ))
                        break ;
                }
            }
            ...
            while ( true ) {
                ...
                if ( pDel->next(0).compare_exchange_strong( p, p | 1, memory_model::memory_order_acquire, CDS_ATOMIC::memory_order_relaxed ))
                {
        ...
        void dispose_chain( position& pos )
        {
            ...
            dispose_chain( m_pDeferredDelChain.exchange( null_ptr<node_type *>(), memory_model::memory_order_release )) ;
        }
        ...
        void defer_chain( position& pos )
        {
            if ( pos.pDelChain ) {
                ...
                node_type * pDeferList ;
                do {
                    pDeferList = m_pDeferredDelChain.load( memory_model::memory_order_relaxed ) ;
                    pTail->m_pDelChain = pDeferList ;
                } while ( !m_pDeferredDelChain.compare_exchange_weak( pDeferList, pHead, memory_model::memory_order_acquire, CDS_ATOMIC::memory_order_relaxed )) ;
