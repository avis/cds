...
namespace cds { namespace container {
    ...
    */
    ...
    protected:
        //@cond
        typedef details::make_split_list_set< cds::urcu::gc< RCU >, T, typename Traits::ordered_list, split_list::details::wrap_set_traits<T, Traits> > options ;
...
#       endif
        ...
        {
#       ifdef CDS_CXX11_LAMBDA_SUPPORT
            return base_class::find_with( val, typename options::template predicate_wrapper<Less>::type(),
                [&f]( node_type& item, Q& val ) { cds::unref(f)(item.m_Value, val) ; } );
#       else
            find_functor_wrapper<Func> fw(f) ;
            return base_class::find_with( val, typename options::template predicate_wrapper<Less>::type(), cds::ref(fw) )   ;
#       endif
        }
...
#       endif
        ...
        */
        ...
        {
             return base_class::erase_with( key, typename options::template predicate_wrapper<Less>::type() ) ;
        }
...
#       else
            erase_functor_wrapper<Func> fw( f ) ;
            return base_class::erase_with( key, typename options::template predicate_wrapper<Less>::type(), cds::ref(fw) ) ;
#       endif
        }
